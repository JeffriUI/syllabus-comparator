...
def sync_dataset(dataset, local_rank, world_size, device):
    ...
    if local_rank == 0:
        for key in integer_keys_2d + integer_keys_3d + float_keys_3d + float_keys_1d:
            ...
            values = dataset[key]
            v_tensor = torch.tensor(values, dtype=dtype).cpu(device)
            shape_tensor = torch.tensor(v_tensor.shape, dtype=torch.int32).cpu(device)
        ...
        for key in integer_keys_2d + integer_keys_3d + float_keys_3d + float_keys_1d:
            ...
            if key in integer_keys_2d:
                shape_tensor = torch.tensor([0, 0], dtype=torch.int32).cpu(device)
            elif key in float_keys_3d or key in integer_keys_3d:
                shape_tensor = torch.tensor([0, 0, 0], dtype=torch.int32).cpu(device)
            else:
                shape_tensor = torch.tensor([0], dtype=torch.int32).cpu(device)

            dist.scatter(shape_tensor, src=0, async_op=False)
            v_tensor = torch.zeros(shape_tensor.tolist(), dtype=dtype).cpu(device)
